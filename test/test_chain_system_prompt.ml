...
let test_chain_llm_args_glm5_code_system_prompt () =
  let system = Some "BOOT: follow instructions" in
  let args =
    Tools_eio.chain_llm_args
      ~timeout:123
  ...
  match args with
  | Types.Glm { model; system_prompt; _ } ->
      check string "glm model" "glm-5-code" model;
      check (option string) "glm system_prompt" system system_prompt
  | _ ->
      fail "expected Types.Glm"
...
let () =
  run "chain_system_prompt" [
    ("chain_llm_args", [
      ...
      test_case "glm-5-code system_prompt" `Quick test_chain_llm_args_glm5_code_system_prompt;
    ]);
  ]
