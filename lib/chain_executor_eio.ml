...
type checkpoint_config = Chain_exec_context_eio.checkpoint_config = {
...
let default_checkpoint_config = Chain_exec_context_eio.default_checkpoint_config
let make_context = Chain_exec_context_eio.make_context
let set_node_status = Chain_exec_context_eio.set_node_status
let next_attempt = Chain_exec_context_eio.next_attempt

(** {1 Safe Helpers from Chain_utils} *)
include Chain_utils

let make_checkpoint_config = Chain_exec_context_eio.make_checkpoint_config
let save_checkpoint = Chain_exec_context_eio.save_checkpoint
let restore_from_checkpoint = Chain_exec_context_eio.restore_from_checkpoint
let node_completed_in_checkpoint = Chain_exec_context_eio.node_completed_in_checkpoint
let store_node_output = Chain_exec_context_eio.store_node_output
let add_trace = Chain_exec_context_eio.add_trace
let record_start = Chain_exec_context_eio.record_start
let record_complete = Chain_exec_context_eio.record_complete
let record_error = Chain_exec_context_eio.record_error
let trace_to_entry = Chain_exec_context_eio.trace_to_entry
let traces_to_entries = Chain_exec_context_eio.traces_to_entries

(** {1 Input Resolution} *)

let resolve_single_input = Chain_exec_inputs_eio.resolve_single_input
let resolve_inputs = Chain_exec_inputs_eio.resolve_inputs
let substitute_prompt = Chain_exec_inputs_eio.substitute_prompt
let substitute_json = Chain_exec_inputs_eio.substitute_json
