...
let rec execute ~sw ~proc_mgr ~clock args : tool_result =
                  ...
                  | first_choice :: _ ->
                    ...
                    (try
                      ...
                      List.iter (fun tc ->
                        ...
                        with Yojson.Safe.Util.Type_error _ -> call_id in
                        ...
                        with Yojson.Safe.Util.Type_error _ -> func_name in
  ...
  | GlmOcr { file; model; timeout; api_key } ->
      ...
      let model_name = sprintf "glm.ocr (%s)" model in
      (* Resolve API key: explicit override > ZAI_API_KEY env var *)
      let resolved_key =
        match api_key with
        | Some k -> Some k
      ...
      | Some api_key ->
          ...
          let cmd_args =
            [
              ...
              "-X";
              "POST";
              "https://api.z.ai/api/coding/paas/v4/layout_parsing";
              "-H";
          ...
          | Ok r -> (
              try
                ...
                if error <> `Null then
                  ...
                  let md_results =
                    Safe_parse.json_string_opt json "md_results"
                    |> Option.value ~default:""
                  in
                  let response =
                    if String.trim md_results <> "" then md_results
                    else Yojson.Safe.pretty_to_string json
                  in
                  let extra =
                    [ ("cloud", "zai"); ("endpoint", "layout_parsing") ]
                  in
                  let extra =
                    match Safe_parse.json_string_opt json "request_id" with
                    | Some rid when rid <> "" -> extra @ [ ("request_id", rid) ]
                    | _ -> extra
                  in
                  let usage = json |> member "usage" in
                  let extra =
                    match Safe_parse.json_int_opt usage "total_tokens" with
                    | Some n -> extra @ [ ("total_tokens", string_of_int n) ]
                    | None -> extra
                  in
                  {
                    model = model_name;
              ...
              with e ->
                {
                  ...
                  returncode = -1;
                  response =
                    sprintf "Error parsing OCR response: %s. Raw: %s"
                ...
                | None ->
                    let result =
                      ...
                      | "ollama_list" ->
                          ...
                          execute ~sw ~proc_mgr ~clock args
                      | "glm" ->
                          let args = parse_glm_args args in
                          execute ~sw ~proc_mgr ~clock args
                      | "glm.ocr" ->
                      ...
                      | "fetch" ->
                          ...
                          if url = "" then
                            ...
                            let curl_result = Cli_runner_eio.run_command ~sw ~proc_mgr ~clock ~timeout:timeout_int
                              "curl" ["-s"; "-L"; "-H"; "Accept: text/markdown, text/html;q=0.9"; url] in
                            (match curl_result with
                            | Error (Cli_runner_eio.Timeout t) ->
                                { Types.model = "fetch"; returncode = -1;
                                  response = sprintf "Error: fetch timed out after %ds" t; extra = [] }
                            | Error (Cli_runner_eio.ProcessError msg) ->
                                { Types.model = "fetch"; returncode = -1; response = sprintf "Error: fetch failed: %s" msg; extra = [] }
                            | Ok r ->
                                { Types.model = "fetch"; returncode = 0; response = r.stdout; extra = [] })
                      | _ ->
                  ...
                  if url = "" then
                    ...
                    | Error (Cli_runner_eio.Timeout t) ->
                        { model = "fetch"; returncode = 1;
                          response = sprintf "Error: fetch timed out after %ds" t; extra = [] }
                    | Error (Cli_runner_eio.ProcessError msg) ->
                        { model = "fetch"; returncode = 1; response = sprintf "Error: fetch failed: %s" msg; extra = [] }
                    | Ok r ->
            ...
            let translation =
              try
                ...
                if List.length choices > 0 then
                  ...
                  r.stdout
              with (Yojson.Safe.Util.Type_error _ | Yojson.Json_error _) -> r.stdout
            in
