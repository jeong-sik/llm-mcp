(** Compact Protocol v0.1-v4 - LLM-to-LLM Communication

    Token-efficient encoding for inter-agent communication.
    Supports multiple compression levels: DSL, Base85, Gzip, Zstd, Dictionary Zstd *)
...
let decode_compact_response s =
  ...
  | "RES" :: status :: model :: tokens :: rest ->
      Some {
        ...
        tokens = Safe_parse.int ~context:"compact_response:tokens" ~default:0 tokens;
